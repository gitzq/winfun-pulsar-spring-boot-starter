        ...
        if (null != serviceUrlMap && !serviceUrlMap.isEmpty()) {
            for (Entry<String, String> entry : serviceUrlMap.entrySet()) {
                ...
                if (StringUtils.isNotBlank(serviceUrl)) {
                    try {
                        Integer ioCount = multiPulsarProperties.getIoThreadsBySourceName(sourceName);
                        ...
                        this.put(sourceName, CustomerPulsarClient.builder().client(client)
                                .maxPendingMessages(multiPulsarProperties.getMaxPendingMessages(sourceName) * ioCount)
                                .build());
